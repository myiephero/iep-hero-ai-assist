...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.use(session({
    ...
    cookie: { 
      secure: false, // Always false for development with Vite
      httpOnly: true,
      maxAge: 24 * 60 * 60 * 1000, // 24 hours
      sameSite: 'lax' // Important for Vite dev server
    }
  }));
...
Use professional, supportive language that empowers the parent while being legally appropriate.`;
  ...
  // Advocate clients endpoint - removed duplicate route, now handled by advocate-clients.ts
...
Focus on functional skills that will help the student succeed in their educational environment.`;
  ...
  app.post("/api/documents/:id/analyze", requireAuth, async (req, res) => {
    try {
      ...
      // Check if document was generated by AI (can't analyze generated documents)
      if (document.generatedBy) {
        return res.status(400).json({ 
          message: 'Cannot analyze generated documents',
          error: 'This document was generated by AI and cannot be re-analyzed'
        });
      }
      
      // Check if document has a physical file
      if (!document.filename || !document.fileUrl) {
        return res.status(400).json({ 
          message: 'Cannot analyze document',
          error: 'Document does not have an associated file to analyze'
        });
      }
      ...
      console.log(`📋 Analyzing document: ${document.originalName}`);
      ...
      if (!fs.existsSync(filePath)) {
        return res.status(404).json({ 
          ...
          error: 'The physical file for this document could not be located'
        });
      }
      
      // Perform AI analysis
      const analysis = await analyzeIEPDocument(filePath, document.originalName);
